...
const iconMap: Record<string, { icon: ReactNode; colorClass: string }> = {
  ...
  'REST API': { icon: <span className="text-gray-300">API</span>, colorClass: 'text-gray-300' },
  ...
  'Styled-Components': { icon: <SiStyledcomponents />, colorClass: 'text-pink-300' },
  ...
  'Immutable.js': { icon: <span className="font-bold text-gray-200 text-[10px]">IM</span>, colorClass: 'text-gray-200' },
...
export default function TechBadge({ name }: TechBadgeProps) {
  ...
  return (
    ...
    >
      {map && (
        <span className={`${map.colorClass} text-[14px]`}>{map.icon}</span>
      )}
